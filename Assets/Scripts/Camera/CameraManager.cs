...
public class CameraManager : MonoBehaviour, ITurnActor
{
    ...
    private const int SHADOWEXTRA = 2;
    ...
    void Awake() {
        ...
        int i = 0;
        GraphicsFormat format = supportedFormats[i];
        while (i < supportedFormats.Count && !SystemInfo.IsFormatSupported(format, GraphicsFormatUsage.Render)) {
            i++;
        ...
        for (i = 0; i<Map.layerLocations.Count; i++) {
            var bounds = Map.LayerBounds[i];

            vRT.Add(new RenderTexture((bounds.size.x+SHADOWEXTRA)*shadowRenderTextureScale,(bounds.size.y+SHADOWEXTRA)*shadowRenderTextureScale, 1, format));
            vRT[i].Create();
            eRT.Add(new RenderTexture((bounds.size.x+SHADOWEXTRA)*shadowRenderTextureScale,(bounds.size.y+SHADOWEXTRA)*shadowRenderTextureScale, 1, format));
            eRT[i].Create();
