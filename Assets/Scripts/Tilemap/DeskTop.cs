        ...
        EditorApplication.delayCall += () => {
            ...
            for (int x = bounds.xMin; x < bounds.xMax; x++)
                for (int y = bounds.yMin; y < bounds.yMax + 1; y++) {
                    ...
                    tiles.Add(Map.IsTile(x, y - 1, WallTile) && Map.IsTile(x, y, WallTile) && Map.Wall.GetTile(new Vector3Int(x, y, 0)) && !Map.Wall.GetTile(new Vector3Int(x, y-1, 0)) ? TopTile : null);
                }
            _thisTilemap.SetTiles(positions.ToArray(), tiles.ToArray());
            Debug.Log($"[DeskTop] Generated {positions.Count} top tiles in batch mode", this);
        };
    }
}