...
namespace HeistGame.Door
{
    ...
    {
        bool IsLocked { get; }
        bool TryUnlock(int itemID); // Returns true if the player has the right key/beats the minigame
        bool TryLock();
    }
    ...
    public interface IDoorDestroyBehavior
    {
        ...
        bool TryDestroy(int itemID); // Returns true if the player has the right tool/beats the minigame
    }
